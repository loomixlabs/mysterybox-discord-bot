...
const NEW_MEMBER_ROLE_ID = '1248150310044831836';
...
async function postEventAnnouncement() {
  ...
  client.once('ready', async () => {
    try {
      ...
      await channel.send({
        content: `<@&${NEW_MEMBER_ROLE_ID}>`,
        embeds: [infoEmbed, rewardsEmbed]
      });

      console.log(`✅ Annonce de l'événement postée dans: ${channel.name}`);
      console.log(`📢 Avec ping du rôle: ${NEW_MEMBER_ROLE_ID}\n`);
