...
async function verifyAimantConfig() {
  try {
    ...
    const USER_ID = '297307186307006464'; // Ton vrai USER_ID

    console.log('🔍 VÉRIFICATION - Configuration Aimant à Légendaires (SERVEUR TEST)\n');
    ...
    if (activeBonus.length === 0) {
      ...
      if (expiresAt) {
        ...
        const timeLeftMinutes = Math.floor((timeLeftSeconds % 3600) / 60);
        console.log(`   Temps restant: ${timeLeftHours}h ${timeLeftMinutes}m ${isExpired ? '❌ EXPIRÉ' : '✅'}`);
      }
      ...
      const checks = {
        ...
        'effect_config.boost_percentage existe': bonus.effect_config?.boost_percentage !== undefined
      };
      ...
      if (allGood) {
        ...
        console.log(`🎯 Boost attendu: legendary ${bonus.effect_config.target_rarity === 'legendary' ? '5%' : '?%'} → ~44% (avec boost de +${bonus.effect_config.boost_percentage})\n`);
      } else {
