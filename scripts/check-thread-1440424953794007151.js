...
async function checkThread() {
  try {
    ...
    const threadId = '1440424953794007151';
    ...
    // Méthode 1: guild.channels.fetch (celle utilisée dans le FIX)
    ...
    try {
      ...
      console.error(`   ⚠️  ATTENDU pour les PrivateThreads!`);
    }
    ...
    try {
      ...
      if (thread2) {
        ...
        console.log(`   Type: ${thread2.type}`);
        console.log(`   Locked: ${thread2.locked || false}`);
      } else {
        console.log('❌ Thread est null');
      }
    } catch (error) {
    ...
    console.log('\n📊 CONCLUSION:');
    console.log('   Les PrivateThreads ne sont PAS accessibles via guild.channels.fetch()');
    console.log('   Il faut utiliser client.channels.fetch() pour les threads privés');
