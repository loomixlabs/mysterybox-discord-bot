...
async function createGuildConfigTable() {
  console.log('🔧 Création de la table guild_config pour la configuration multi-serveur\n');
  console.log('━'.repeat(100));

  try {
    // 1. Créer la table guild_config
    console.log('\n📊 ÉTAPE 1: Création de la table guild_config\n');
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS guild_config (
        ...
        modules_enabled JSONB DEFAULT '["mysterybox", "missions", "leaderboard"]'::jsonb,
    ...
    console.log('   ✅ Table guild_config créée\n');
    ...
    const configs = await db.query(`
      SELECT guild_id, bot_display_name, embed_footer_text
      FROM guild_config
      ORDER BY guild_id
    `);
